...
import { useState, useEffect, useRef, type FormEvent, type ChangeEvent, type DragEvent, use } from 'react'
...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    Promise.all([
      ...
      .then(([prodData, catData]) => {
        ...
        if (prodData.product) {
          ...
          const loadedImages = p.images?.length ? p.images : (p.image ? [p.image] : [])
  ...
  // Drag and drop reordering for main images
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      success(`Updated "${form.name}" successfully!`)
  ...
  if (loading) {
    return <div className="p-12 text-center text-sm text-muted-foreground animate-pulse">Loading product details…</div>
  }
    ...
    <div className="flex-1 min-w-0 pb-16">
      <header className="flex h-16 items-center justify-between border-b border-border bg-card px-5 lg:px-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-muted-foreground uppercase tracking-widest font-semibold">Catalog / Edit</p>
            <h1 className="font-serif text-xl">Edit Product: {form.name}</h1>
          </div>
        </div>
        <button
          type="submit"
        ...
        >
          <Save size={16} /> {submitting ? 'Updating…' : 'Save Changes'}
        </button>
      </header>

      <div className="mx-auto max-w-5xl p-5 lg:p-8">
        <form id="edit-product-form" onSubmit={handleSubmit} className="grid gap-8 lg:grid-cols-[1fr_340px]">
            ...
            <div className="border border-border bg-card p-6 space-y-4">
              <div className="flex items-center justify-between border-b border-border pb-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Manage color, size, or custom variants with variant-specific images and pricing.
                  </p>
                </div>
                <button
                  type="button"
